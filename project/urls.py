...
schema_view = get_schema_view(
    openapi.Info(
        title="API for WINK",
        url="http://127.0.0.1:8000/api/v1/wink",
        description="""При написании сценария важно придерживаться выбранного возрастного рейтинга, устанавливаемого на
         основании норм Возрастной классификации информационной продукции. Такой подход гарантирует попадание контента в
          оптимальные эфирные/кинопрокатные слоты и доступность на стриминговых платформах, что позволяет достичь целевой
           аудитории зрителей. Автоматическая проверка рейтинга по сценарию позволяет авторам и продюсерам заранее понимать,
            где текст может выйти за рамки выбранной категории - делая процесс производства быстрее, прозрачнее и экономичнее.\n
        **API для первых задач**
...
urlpatterns = [
    ...
    path("", include((urls_wink, "wink"), namespace="wink")),
    path("api/v1/", include((api_wink, "api_v1"), namespace="api_v1")),
