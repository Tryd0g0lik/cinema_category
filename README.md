...
Загруженный на сервер файл отправляет рефернный ключ от файла на сторону AI/парсера/анализатора.\
По рефер-ключу начинает качаться файл для анализа.\
!! Проверка на дубли не включена. Возможно дублирование файлов. Если не против алгоритма "`md5`" можно создать проверку дублей.  

Массово, один файл может качать 10 пользователей.\
Рефер-ключ обновляется каждые 90 секунд.\
Останавливает обновление через 10 минут или как только парсер начал качать файл.
!! Логики для перезапуски ключа - нет. 
...
JWT-токен не рассматривается в логике (не доступны ограничения/права).\
...
- [удаляем обращение](https://github.com/Tryd0g0lik/cinema_category/blob/6729d261c59e4dd98cf14259d6b0ed2c96e5a5b1/wink/tasks/task_start_rotation.py#L127) к файлу если оно привышает лимит обращений к обному файлу;
- [колькулятор обращений](https://github.com/Tryd0g0lik/cinema_category/blob/6729d261c59e4dd98cf14259d6b0ed2c96e5a5b1/wink/tasks/task_start_rotation.py#L109) к файлу;
