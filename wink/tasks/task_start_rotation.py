...
"""
# per-rotator lock
RUN_LOCK_KEY = "rotator:run_lock:{inner_pk}"
# per-rotator stop flag
STOP_FLAG_KEY = "rotator:stop_lock:{inner_pk}"
# per-file active session
ACTIVE_COUNT_KEY = "rotator:active_count:{file_pk}"
# mark that session started
ROTATION_MARK_KEY = "rotator:mark_count:{file_pk}"
...
def start_rotation(inter_pk: int, interval: int = 90, duration: int = 600) -> bool:
    ...
    try:
        try:
            ...
            log.error("IntermediateFilesModel pk=%s not foud", inter_pk)
            r.delete(lock_key)
            return False
        """
        ...
        if count > MAX_CONCURRENT:
            try:
                ...
                log.exception("Failed safe-decr for file_pk=%s", file_pk)
            r.delete(lock_key)
            log.info(
                "Active session limit reached for file_pk=%s (max=%s). Aborting start.",
                (file_pk, MAX_CONCURRENT),
            )
            return False
        ...
        while time.time() < end_time:
            ...
            if r.get(stop_key):
                log.info(
                    "Stop flag observed for inter_pk=%s, exiting rotator loop", inter_pk
                )
                break
            ...
            try:
                ...
                log.info("IntermediateModel gone for inter_pk=%s, exiting", inter_pk)
                break
            # CURRENT REFER
            ...
            try:
                ...
                if updated == 0:
                    log.error("inter_pk=%s refer updated -> %s", (inter_pk, new_uuid))
                else:
                    log.info(
        ...
        log.info("Rotator finished for inter_pk=%s", inter_pk)
    finally:
        ...
        try:
            ...
            log.exception("Redis cleanup failed keys for inter_pk=%s", inter_pk)
...
def stop_rotation(inter_pk: int, enough: int = 300) -> bool:
    ...
    try:
        ...
        log.info("Stop flag set for inter_pk=%s", inter_pk)
    except Exception:
        log.exception("Failed to set stop flag for inter_pk=%s", inter_pk)
        return False
    return True
